...
$$ \hat \lambda = exp(\beta_0 + \beta_1 \phi(x_1) + \dots + 
\beta_n \phi(x_n))$$
$$log(\hat \lambda) = \beta_0 + \beta_1 \phi(x_1) + \dots + 
\beta_n \phi(x_n) $$


Or, in words, that the $log(\hat \lambda)$ is equal to a sum of the
...
$log(\hat \lambda)$ when we add one unity of the each variable
(e.g. $\beta_1$ measures the difference that happens in 
$log(\hat \lambda)$ when $x_1$ increases
by one). 
